...
SBomber::SBomber() : exitFlag(false), startTime(0), finishTime(0),
                     deltaTime(0), passedTime(0), fps(0), bombsNumber(10), score(0)
{
...
void SBomber::CheckBombsAndGround()
{
    ...
    for (size_t i = 0; i < vecBombs.size(); i++)
    {
        if (vecBombs[i]->GetY() >= y) // Пересечение бомбы с землей
        {
            ...
            auto pCommand = new DeleteDynamicObjCmd(vecDynamicObj, vecBombs[i]);      // DeleteDynamicObj(vecBombs[i]);
            pCommand->Execute();//vecDynamicObj.push_back(pCommand);
        }
    }
...
void SBomber::CheckDestoyableObjects(Bomb* pBomb)
{
    ...
    for (size_t i = 0; i < vecDestoyableObjects.size(); i++)
    {
        ...
        if (vecDestoyableObjects[i]->isInside(x1, x2))
        {
            ...
            auto pCommand = new DeleteStaticObjCmd(vecStaticObj, vecDestoyableObjects[i]);
            pCommand->Execute();//vecCmds.push_back(pCommand);
        }
    }
}
...
void SBomber::ProcessKBHit()
{
    ...
    switch (c) {
        ...
        case 'B':
        {
            ...
            auto pCommand = new DropBombCmd(pPlane, vecDynamicObj, bombsNumber, score);
            pCommand->Execute();//vecCmds.push_back(pCommand);
            break;
        }
        default:
